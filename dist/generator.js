...
class WebAssemblyGenerator {
    ...
    func(name, params, result, body) {
        const plist = Object.entries(params).map(entry => `(param $${entry[0]} ${types_1.w[entry[1]]})`);
        const res = result ? [`(result ${types_1.w[result]})`] : [];
        this.closure(["func", `$${name}`, ...plist, ...res], [body]);
    ...
    remainder(type, left, right) {
        this.closure([`${types_1.w[type]}.rem`], [left, right]);
    }
